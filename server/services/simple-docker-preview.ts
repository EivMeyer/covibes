...
class SimpleDockerPreview {
  private previews = new Map<string, PreviewStatus>();
  private basePort = 5000;

  async startPreview(teamId: string): Promise<{ port: number; url: string }> {
...
<body>
    ...
    <script>
      ...
      const { stdout } = await execAsync(
        `docker run -d --rm --name preview-${teamId} -p ${port}:80 -v ${contentDir}:/usr/share/nginx/html:ro nginx:alpine`
      );
  ...
  getPreviewStatus(teamId: string): PreviewStatus | null {
