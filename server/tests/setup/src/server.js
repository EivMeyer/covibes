...
dockerManager.on('pty-cleaned', (data) => {
...
dockerManager.on('pty-data', async (data) => {
...
dockerManager.on('pty-exit', (data) => {
...
const globalSSHSessions = new Map();
...
io.on('connection', (socket) => {
    ...
    socket.on('join-team', async (data) => {
        try {
            ...
            const onlineUsersList = Array.from(teamOnlineUsers.get(data.teamId)?.values() || []).map(u => ({
                id: u.id,
                name: u.name,
                email: u.email,
    ...
    socket.on('activity-ping', async (data) => {
    ...
    socket.on('terminal_connect', async (data) => {
        ...
        try {
            ...
            */
            ...
            socket.emit('terminal_error', {
                agentId: data.agentId,
                error: `Failed to establish PTY connection: ${error.message}`
            });
        }
    });
    ...
    socket.on('terminal_disconnect', async (data) => {
        if (!socket.userId)
            return;
        try {
            ...
            const sshSessions = socket.sshSessions;
    ...
    socket.on('preview-refresh', async (data) => {
    ...
    socket.on('preview-status', async (data) => {
        if (!socket.teamId) {
            socket.emit('error', { message: 'Not authenticated' });
            return;
        }
    ...
    socket.on('preview-stop', async (data) => {
        if (!socket.teamId || !socket.userId) {
            socket.emit('error', { message: 'Not authenticated' });
            return;
        }
        try {
            ...
            // Find container
            const container = await prisma.previewContainer.findFirst({
                where: {
                    teamId: socket.teamId,
                    branch: data.branch
                }
            ...
            // Update database
            await prisma.previewContainer.update({
    ...
    socket.on('workspace-update', async (data) => {
        ...
        try {
            ...
            if (data.tiles !== undefined)
                updateData.workspaceTiles = data.tiles;
            if (data.layouts !== undefined)
                updateData.workspaceLayouts = data.layouts;
            if (data.sidebarWidth !== undefined)
                updateData.sidebarWidth = data.sidebarWidth;
    ...
    socket.on('workspace-drag-start', (data) => {
        if (!socket.teamId || !socket.userId) {
            return;
        }
    ...
    socket.on('workspace-drag-move', (data) => {
        if (!socket.teamId || !socket.userId) {
            return;
        }
    ...
    socket.on('workspace-drag-stop', (data) => {
        if (!socket.teamId || !socket.userId) {
            return;
        }
    ...
    socket.on('workspace-tile-add', (data) => {
        if (!socket.teamId || !socket.userId) {
            return;
        }
    ...
    socket.on('workspace-tile-remove', (data) => {
        if (!socket.teamId || !socket.userId) {
            return;
        }
    ...
    try {
        ...
        if (previewMatch) {
            ...
            try {
                ...
                const containers = await dockerManager.getTeamContainers(teamId);
                const previewContainer = containers.find(c => c.type === 'preview' &&
                    c.status === 'running' &&
                    c.previewPort);
                ...
                const wsProxy = createProxyMiddleware({
                    target: `http://localhost:${previewContainer.previewPort}`,
                    changeOrigin: true,
                    ws: true,
                    pathRewrite: {
...
async function startServer() {
    try {
        ...
        server.listen(PORT, () => {
            ...
            console.log(`📱 Mobile view at: http://localhost:${PORT}/mobile`);
