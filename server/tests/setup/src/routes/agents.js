 ...
 */
async function executeAgentAsync(agentId, vmId, type, task, terminalLocation, terminalIsolation, repositoryUrl, userId, teamId) {
    try {
        ...
        try {
            ...
            if (terminalIsolation === 'docker') {
                ...
                const terminalSession = await manager.spawnTerminal({
                    agentId,
                    userId: userId,
                    teamId: teamId,
                    task,
...
router.post('/spawn', async (req, res) => {
    try {
        ...
        if (!user.vmId) {
            ...
            console.log(`Auto-assigned default VM to user ${req.userId}`);
        }
        ...
        executeAgentAsync(agent.id, user.vmId, agentType, task, terminalLocation || 'local', terminalIsolation || 'none', user.teams?.repositoryUrl || null, req.userId, user.teamId);
        ...
        if (io) {
            ...
            io.to(user.teamId).emit('agent-spawned', {
                agent: {
                    ...
                    outputLines: 0,
                    isOwner: req.userId === agent.userId,
                    output: agent.output || ''
                }
            });
        }
...
router.get('/:id/container', async (req, res) => {
    try {
        ...
        res.json({
            ...
            container: {
                ...
                status: liveStatus?.status || containerInstance.status,
...
router.post('/:id/container/exec', async (req, res) => {
    try {
        const { id: agentId } = req.params;
...
router.get('/:id/terminal-history', async (req, res) => {
    try {
        ...
        const history = await prisma.terminalHistory.findMany({
            where: {
                agentId: agentId
            },
            orderBy: {
...
router.get('/:agentId/claude-status', authenticateToken, async (req, res) => {
    try {
        ...
        const agent = await prisma.agents.findFirst({
            where: {
                ...
                OR: [
                    { userId: req.userId },
                    { teams: { users: { some: { id: req.userId } } } }
                ]
            },
    ...
    try {
        ...
        const agent = await prisma.agents.findFirst({
            where: {
                id: agentId,
                OR: [
                    { userId: req.userId }, // User's own agent
...
router.post('/:id/reconnect', async (req, res) => {
    try {
        ...
        try {
            ...
            const session = await manager.spawnTerminal({
                agentId: agent.id,
                userId: req.userId,
                teamId: user.teamId,
                task: agent.task,
                location: 'local',
                isolation: 'tmux',
                workspaceRepo: agent.repositoryUrl || undefined
            });
...
router.get('/tmux-sessions', async (req, res) => {
    try {
        ...
        if ('listColabVibeSessions' in manager) {
            const sessions = await manager.listColabVibeSessions();
