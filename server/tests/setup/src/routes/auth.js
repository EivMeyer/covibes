 ...
 */
const generateToken = (userId, teamId) => {
    return jwt.sign({ userId, teamId }, JWT_SECRET, { expiresIn: '24h' });
};
/**
...
router.get('/github/callback', (req, res, next) => {
    passport.authenticate('github', (err, user, info) => {
        ...
        if (user?.id && user?.teamId) {
            console.log('Existing user found - generating token', { userId: user.id, teamId: user.teamId });
            const token = generateToken(user.id, user.teamId);
