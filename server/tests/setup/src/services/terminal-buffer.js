...
class TerminalBufferService {
    constructor() {
        ...
        this.MAX_BUFFER_SIZE = 1000; // Maximum entries per terminal
        this.SESSION_TIMEOUT = 30 * 60 * 1000; // 30 minutes inactive cleanup
        this.CLEANUP_INTERVAL = 5 * 60 * 1000; // 5 minutes cleanup check
     ...
     */
    initializeSession(agentId, width = 80, height = 24) {
     ...
     */
    getRecentHistory(agentId, options = {}) {
        const session = this.sessions.get(agentId);
        if (!session) {
            return [];
        }
     ...
     */
    addSubscriber(agentId, socketId, userId) {
        const session = this.initializeSession(agentId);
        const subscriber = {
            socketId,
            userId,
     ...
     */
    removeSubscriber(agentId, socketId) {
        const session = this.sessions.get(agentId);
        if (!session) {
            return false;
        }
     ...
     */
    removeSubscriberFromAll(socketId) {
     ...
     */
    resizeTerminal(agentId, width, height) {
        const session = this.sessions.get(agentId);
        if (session) {
