...
class UniversalPreviewService {
    ...
    async validateContainerHealth(containerName) {
        try {
    ...
    async cleanupDeadPreview(teamId) {
    ...
    async startPreview(teamId, repositoryUrl) {
        ...
        if (existing) {
            ...
            if (isHealthy) {
                ...
                return {
                    ...
                    url: `http://localhost:${existing.proxyPort}`
                };
            }
...
CMD ["npm", "run", "dev", "--", "--host", "0.0.0.0", "--port", "5173"]`;
            ...
            const { stdout } = await execAsync(`docker run -d --rm ` +
                `--name preview-${teamId} ` +
                `-p ${port}:5173 ` +
                `-v ${projectDir}:/app ` +
            ...
            return {
                ...
                url: `http://localhost:${proxyPort}` // Direct URL like MVP
            };
        }
        catch (error) {
    ...
    async updateViteConfigWithProxy(teamId, proxyPort) {
        const projectDir = path.join(this.workspaceDir, teamId);
...
export default defineConfig({
  ...
  server: {
    ...
    async cloneRepositoryWithFallback(repositoryUrl, targetDir) {
    ...
    async createViteReactProject(projectDir, teamId, port) {
        const proxyBasePath = `/api/preview/${teamId}/workspace/`;
...
.version {
  ...
  font-size: 0.9rem;
    ...
    async createSampleProject(projectDir) {
        console.log('📝 Creating sample project...');
        await fs.mkdir(projectDir, { recursive: true });
        // Create a simple Vite + React project structure
        const packageJson = {
            "name": "preview-app",
            "type": "module",
            "scripts": {
...
function App() {
  ...
  return (
    <div style={{
      ...
      <h1>🚀 Universal Preview</h1>
      <p>Edit src/App.jsx and save to see hot reload in action!</p>
      <div style={{ fontSize: '48px', margin: '40px 0' }}>
        {count}
      </div>
      <button 
...
EXPOSE ${projectInfo.port}
CMD ["sh", "-c", "while true; do ${projectInfo.command}; sleep 1; done"]`;
        }
        // Node.js based projects
        return `FROM node:20-alpine
WORKDIR /app

# We'll mount the source code as a volume, so dependencies need to be installed at runtime
EXPOSE ${projectInfo.port}

# Install dependencies and run dev server
# The && ensures npm install runs before the dev command
CMD ["sh", "-c", "npm install --include=dev && ${projectInfo.command}"]`;
    }
    async stopPreview(teamId) {
        ...
        try {
            ...
            console.log(`✅ Stopped and removed preview container for team ${teamId}`);
        }
        catch (error) {
            console.log(`Preview container for ${teamId} already stopped`);
        }
        // Stop dedicated proxy server
        await dedicatedPreviewProxy.stopProxy(teamId);
    ...
    async stopPreview(teamId) {
        ...
        try {
            ...
            console.log(`⚠️ Container might already be stopped: ${error.message}`);
        }
