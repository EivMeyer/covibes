...
class LocalPreviewService {
    constructor() {
        ...
        this.portStart = 4000;
        this.maxPreviews = 100;
        this.baseWorkspaceDir = path.join(os.homedir(), '.colabvibe', 'previews');
     ...
     */
    async createViteProject(workspaceDir, port, teamId) {
...
log("✅ Application initialized successfully")`;
     ...
     * Start Vite dev server
     */
    async startViteServer(workspaceDir, port) {
    ...
    async shutdown() {
        ...
        const stopPromises = Array.from(this.previews.keys()).map(teamId => this.stopPreview(teamId));
