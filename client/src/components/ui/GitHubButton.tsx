...
export const GitHubButton: React.FC<GitHubButtonProps> = ({
  ...
  const handleClick = () => {
    if (onClick) {
      ...
      // Use the backend URL directly since OAuth requires a full page redirect
  ...
  return (
    <Button
      onClick={handleClick}
      disabled={disabled}
      className={`bg-gray-800 hover:bg-gray-700 text-white flex items-center justify-center gap-2 ${className}`}
      type="button"
    >
      <svg
        className="w-5 h-5"
